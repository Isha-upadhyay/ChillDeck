...
import { generateOutline, exportSlides, generateOutlineStream } from "@/lib/api";
...
export default function Home() {
  ...
  const [streamProgress, setStreamProgress] = useState<{ step: string; message: string; progress: number } | null>(null);
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      if (folderId) {
        ...
        router.push(
          `/slides/${res.presentation_id}?theme=${selectedTheme}${
  ...
  return (
    <main className="min-h-screen bg-[#0b0b0f] text-white px-6 py-16 flex justify-center">
      ...
      `}
          ...
          {error && (
            <p className="mt-4 text-red-400 bg-red-900/20 p-3 rounded-lg text-sm">
              {error}
            </p>
          )}
        </div>
        ...
        {/* GENERATION PROGRESS */}
        {streamProgress && (
          <div className="mt-6 bg-[#11111a] border border-gray-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-300">{streamProgress.message}</span>
              <span className="text-sm text-indigo-400">{streamProgress.progress}%</span>
            </div>
        ...
        {result && result.slides && (
          <div className="mt-10">
            ...
            <div className="bg-[#11111a] border border-gray-800 p-5 rounded-xl shadow flex flex-wrap gap-3 mb-6">
              <Button
                onClick={() =>
                  exportSlides(result.slides, result.topic, "pptx")
                }
              ...
              <Button
                onClick={() =>
                  exportSlides(result.slides, result.topic, "json")
                }
